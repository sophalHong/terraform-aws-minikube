...
export KUBERNETES_VERSION="1.23.1"
...
yum install -y bash-completion yum-utils curl gettext device-mapper-persistent-data lvm2
...
yum install -y kubelet-$KUBERNETES_VERSION kubeadm-$KUBERNETES_VERSION kubernetes-cni --disableexcludes=kubernetes --nobest
...
- groups:
  ...
  token: `tr -dc a-z0-9 </dev/urandom | head -c 6 ; echo ''`.`tr -dc a-z0-9 </dev/urandom | head -c 16 ; echo ''`
  ttl: 0s
  usages:
  - signing
  - authentication
